...
| **TubInfo** | A card showing the Collection's own name, subtitle, description and image. Note the Collection binding prefix is `tub.`, not `collection.` — the entity was renamed, the namespace was not, and `{{collection.name}}` silently resolves to nothing |
