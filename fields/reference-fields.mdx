...
Collection binding rather than a reference field: `{{tub.name}}`. **The Collection binding
prefix is still `tub.`, not `collection.`** — Collections were renamed, the binding namespace
was not. `{{collection.name}}` returns an empty string with no error.
