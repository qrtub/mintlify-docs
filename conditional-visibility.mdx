...
description: "Show or hide Destinations on Pages based on conditions"
sidebar_position: 4
---

Conditional Visibility lets you control which Destinations appear on Pages based on Item data or the device someone is using. This is an advanced feature for complex routing scenarios—most users won't need it.
...
**How to set this up:**
1. Use the standard `type` field or add a custom field (e.g., `equipmentType`) to your Collection
...
**How to set this up:**
1. Add a `testStatus` field to your Collection
...
### iOS Safari Workaround (Mitti Example)

**Scenario:** Mitti (formerly SafetyCulture) app deep links (`iauditor://`) only work in Safari on iOS. You need to show the web version for iOS users in other browsers.
...
- **Custom fields** - Any fields you've defined in your Collection: `item.{yourFieldName}`
- **Collection fields** - `tub.name`, `tub.metadata.organizationName`, etc. (the prefix is `tub.`, not `collection.` — Collections were formerly called Tubs)
...
**See [Using Fields](/using-fields) for complete field reference with all available fields, types, and examples.**
...
See [Device Detection](/device-detection) for device routing patterns and examples.
...
- [Using Fields](/using-fields) - Complete field reference with all available fields
- [Device Detection](/device-detection) - Device-based routing and available device fields
