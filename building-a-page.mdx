...
one layout for a Collection, and every Item in that Collection renders it with its own data.

## Before you start

Pages must be switched on for the Collection. Open the Collection's settings and turn on the page option
...
From the Collection's settings, open the **Profile page** tab and choose **Edit profile page**. The
...
onto the setting you want it in. Beyond Item and Collection fields, you can bind device and time
information — see [Using Fields](/using-fields) for the full reference.

Note the Collection prefix is `tub.`, not `collection.` — Collections were formerly called Tubs, and the binding names kept the original term.
...
- Any real Item in the Collection uses that Item's actual field values
...
## Saving: the whole Collection, or one Item
