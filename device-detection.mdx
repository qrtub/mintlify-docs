...
**Note:** If your concern is simply "what if the app isn't installed?", the [Fallback URL feature](/app-links) handles that automatically — no device routing needed. Device routing is for the specific case where the app is installed but the browser blocks deep links (iOS Chrome/Firefox).
...
- [App Links & Fallback URLs](/app-links) - Automatic fallback when a mobile app isn't installed
- [Using Fields](/using-fields) - Complete field reference including device fields
- [Conditional Visibility](/conditional-visibility) - Using device detection in visibility conditions
- [Pages Overview](/pages-overview) - Setting up Pages with multiple Destinations
