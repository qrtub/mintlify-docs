...
description: "The four general settings — name, what one Item is called, description and cover image — where each one shows up, and their tub.* bindings"
---
...
`{{tub.items_name}}` binding used as the subtitle on the built-in default page. Leaving it blank
...
| Binding | Value |
|---------|-------|
| `{{tub.name}}` | The Collection name |
| `{{tub.description}}` | The description |
| `{{tub.items_name}}` | The "what is one Item called" word |
| `{{tub.image_url}}` | The cover image URL |

**The namespace is `tub.`, not `collection.`.** Collections were renamed from Tubs and the
binding prefix still uses the original word. `{{collection.name}}` is not an error — it resolves
to an empty string, silently, which is exactly why it is worth stating.

One caution if you put `{{tub.name}}` inside a Destination URL: values are inserted exactly as
