...
- Collection fields use the `tub.` prefix, not `collection.` — `{{tub.name}}` returns the
  Collection's name. Collections were renamed from Tubs and the binding namespace was not, so
  `{{collection.name}}` silently returns an empty string with no error.
