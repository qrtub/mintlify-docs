...
**Note:** If your concern is simply "what if the app isn't installed?", the [Fallback URL feature](/help/app-links) handles that automatically — no device routing needed. Device routing is for the specific case where the app is installed but the browser blocks deep links (iOS Chrome/Firefox).
...
### 4. Tablet-Optimised Experience

**Scenario:** You have a special interface optimised for tablets.
...
**Result:** Tablet users get the optimised experience automatically.
...
- Optimised experiences per device type

## Related

- [App Links & Fallback URLs](/help/app-links) - Automatic fallback when a mobile app isn't installed
...
- [Pages Overview](/help/pages-overview) - Setting up Pages with multiple Destinations
