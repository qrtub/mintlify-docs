...
| `tub` | The Collection the Item belongs to — see [Collection Fields Reference](/destinations/collection-fields) | Destinations and Pages |
...
| `accessUrl` | The specific Link that was scanned: its slug, strategy and sequence number | Destinations only |
...
Two of those names do not match the words used in the interface, because the underlying data
was renamed and the binding namespaces were not:

- **The Collection prefix is `tub.`, not `collection.`.** Collections were previously called
  Tubs. Write `{{tub.name}}`. `{{collection.name}}` is not a namespace and resolves to nothing,
  with no error message.
- **The Link prefix is `accessUrl.`, not `link.`.** Same reason.
