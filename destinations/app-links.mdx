...
Configure both once. Roll out to 500 items. Each scan routes to the right app screen (or web equivalent) for that specific item — no per-item configuration.
...
See the [Mitti Integration guide](/integrations/mitti/setup) for full setup instructions.
...
- [Mitti Integration](/integrations/mitti/setup) — full deep link and fallback setup for iAuditor
