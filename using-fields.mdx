...
description: "Reference Item data, Collection information, user session, and device details in URLs and conditions"
sidebar_position: 3
---

QRtub Pages can dynamically use data from your Items, Collections, user sessions, and device information. This guide shows you how to reference fields in URL Templates and conditional visibility expressions.
...
Any custom fields you define in your Collection are accessible as `item.{fieldName}`.

**Example:**
If you add a custom field called `inspectionDue` to your Collection, reference it as:
- URL Template: `{{item.inspectionDue}}`
- Condition: `item.inspectionDue != ""`

### Collection Fields

Access information about the Collection containing the Item.

**The binding prefix is `tub.`, not `collection.`** — Collections were previously called Tubs, and the binding names still use the original term. Write `tub.name`, not `collection.name`. Everything else about them works exactly as documented here.

| Field | Type | Description | Example |
|-------|------|-------------|---------|
| `tub.id` | string | Collection unique identifier | `"tub-123"` |
| `tub.name` | string | Collection display name | `"Heavy Equipment"` |
| `tub.description` | string | Collection description | `"Construction equipment fleet"` |
| `tub.image_url` | string | Collection image URL | `"/images/equipment-icon.png"` |
| `tub.items_name` | string | Custom label for this Collection's Items | `"Machines"` |
| `tub.created_at` | date | Collection creation date | `"2024-01-01T00:00:00Z"` |
...
See [Device Detection](/device-detection) for detailed device routing examples.

### Theme Fields

Access Page theme configuration.
...
See [Conditional Visibility](/conditional-visibility) for more complex condition examples.
...
- Custom fields: Whatever name you gave them in Collection configuration
- Case-sensitive: `item.Status` will NOT work, use `item.status`

**URL Encoding:**
- Field values are inserted into URL templates exactly as stored
- If a value may contain spaces or characters such as `&`, `?` or `/`, make sure the receiving system accepts them
...
1. **Use AI to generate expressions** - See [Conditional Visibility](/conditional-visibility#using-ai-to-generate-conditions)
2. **Test with sample Items** - Create test Items with different field values
3. **Check field names** - Verify exact field names in your Collection configuration
4. **Contact support** - Email [hi@qrtub.com](mailto:hi@qrtub.com) with your use case

## Related

- [Conditional Visibility](/conditional-visibility) - Show/hide Destinations based on conditions
- [Device Detection](/device-detection) - Device-based routing
...
- [Pages Overview](/pages-overview) - Understanding Pages and Destinations
