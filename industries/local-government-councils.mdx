...
icon: "landmark"
description: "QR codes on council assets for inspections, contractor access and public reporting"
---
...
Every council asset gets a single QR code. Everyone who scans sees the SAME Page with ALL the buttons. Each person taps what's relevant to them.

**Example Page (Playground Equipment):**
...
**Note on security:** The buttons are visible, but they're still protected. If someone taps "Playground Safety Inspection," they reach your inspection app which requires login credentials. The Page shows what's available; your existing systems control who can actually access them.
...
**QRtub Page:** Shows asset info + operational legitimacy + direct reporting + transparency. Public trust.

## Multi-Department, Unified Access

Different council departments, one system:

**Parks & Reserves Collection:**
...
**Buildings & Facilities Collection:**
...
## Rolling Out Across Asset Population

Roll out QR codes across entire council asset base:

**URL Templates example:**
```
yourinspectionapp.com/asset?id={{item.assetID}}&type={{item.assetType}}&location={{item.location}}&department={{item.department}}
```
...
Council tags playground equipment in a community park. Everyone sees the same Page:
...
- The asset's page, with the Destinations you have set up
- "Log Service Completion", opening the system you nominate
- Contact details for the council asset officer

**Council benefits:**
- Contractors reach the right form without training on council systems
- No contractor logins to provision in QRtub
- Easy to switch contractors (update the Destination, not physical tags)
...
URL Templates auto-populate asset data—configure once, install across asset population.
...
3. **Create Collections** - Separate Collection per department or asset category
4. **Define asset fields** - Asset ID, Type, Location, Installed date, Responsible officer, Inspection due
5. **Configure Pages** - Inspection/maintenance Destinations, public reporting, asset information
6. **Roll out** - Import asset inventory, each asset inherits appropriate template
...
- Department-isolated Collections with cross-visibility options
- Pages with public transparency + operational access
